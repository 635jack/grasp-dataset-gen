...
def generate_dataset(config: Optional[DatasetConfig] = None):
    ...
    for glb_path in tqdm(glb_files, desc="Processing objects"):
        ...
        try:
            # 1. Load geometry for raycasting (fast, no colors)
            print(f"\n📦 Loading {mesh_name}...")
            mesh = load_glb(glb_path)
            mesh = normalize_mesh(mesh)

            # 2. Render RGB with native materials/textures (fallback to gray)
            rgb_path = os.path.join(out_dir, "rgb.png")
            print(f"  📷 Rendering images...")
            color_image, depth_map = renderer.render_colored(glb_path, output_path=rgb_path)

            # 2b. Surface visibility score
            surface_vis = calculate_surface_visibility(mesh, config.camera, depth_map)

            # 3. Generate grasps for each strategy
            entry = {
                "mesh": mesh_name,
                "rgb": rgb_path,
    ...
    print(f"   {len(dataset_index)} objects processed, {skipped} skipped")
